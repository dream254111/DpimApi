...
namespace DpimProject.Models.Report
{
    public class ReportStudent
    {
        ...
        public dynamic get_overview_website(ref string error_message)
        {
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    var total_passed_student = 0;
                    foreach(var item in db.course.Where(w => w.is_deleted == 0))
                    {
                        var get_exam = db.course_exam.Where(w => w.is_deleted == 0 && w.course_id == item.id);
                        var get_percent = db.course.Where(w => w.id == item.id).Select(s => s.passed_percent).FirstOrDefault();
                        var passed_percent = ((double)get_percent / 100);
                        var exam_pass = Math.Floor(get_exam.Count() * passed_percent);
                        var count_pass = (from s_info in db.student_course_info
                                    join s in db.student.Where(w => w.is_deleted == 0) on s_info.student_id equals s.id
                                    where s_info.course_id == item.id
                                    select new
                                    {
                                    ...
                                    }).Where(w => w.score >= exam_pass && exam_pass > 0).Count();

                        total_passed_student += count_pass;
                    }
                    ...
                    var data = new
                    {
                        ...
                        total_vdo = db.video_on_demand.Where(w => w.is_deleted == 0).Count(),
                        total_passed_student = total_passed_student
                    };
        ...
        public dynamic get_desc_student(int filter, string course_id, ref string error_message)
        {
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    if (filter == 1)
                    {
                        var mockData = (from p in db.provinces
                                select new
                                {
                        ...
                        var getTop = new List<modelProvince>();
                        foreach(var item in mockData.Where(w => w.count > 0).OrderByDescending(o => o.count).Skip(0).Take(9).ToList())
                        {
                            getTop.Add(item);
                        }

                        var getOther = mockData.Where(w => w.count > 0).OrderByDescending(o => o.count).Skip(10).Take(80).ToList();
                        if(getOther.Count() > 0)
                        {
                            int countOther = getOther.Sum(s => s.count);
                            string countPercent = (((decimal)countOther / get_info.Count()) * 100).ToString("0.##");
                            getTop.Add(new modelProvince { name = "อื่นๆ", count = countOther, percent = countPercent });
                        }
        ...
        public dynamic get_student_by_id(int? user_id, int? student_id, ref string error_message)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                                ...
                                select new
                                {
                                    ...
                                    course = (from s_info in db.student_course_info
                                              ...
                                              where s_info.student_id == s.id 
                                                    && (get_role == 3 ? c.id != null : db.group_department.Where(w => w.user_id == user_id.ToString() && w.depart_id == c.department_id).Count() > 0)
                                              select new
                                              {
                                                  id = c.id,
        ...
        public dynamic EvaluationReport(string course_id, DateTime? start_date, DateTime? end_date, ref string error_message)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    var get_eva_result = (start_date != null ? db.course_evaluation_result.Where(w => w.course_id == course_id && w.is_deleted == 0 && w.created_dt >= start_date && w.created_dt <= end_date) : db.course_evaluation_result.Where(w => w.course_id == course_id && w.is_deleted == 0));
        ...
        public dynamic get_certificate_print_report_by_category(int? user_id, string category_id, DateTime? start_date, DateTime? end_date, ref string error_message)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    var data = (from c in db.course
                                where (category_id == "0" ? c.course_category_id != null : c.course_category_id == category_id)
                                      && c.is_deleted == 0
                                      && (get_role == 3 ? c.id != null : db.group_department.Where(w => w.user_id == user_id.ToString() && w.depart_id == c.department_id).Count() > 0)
                                select new
                                {
        ...
        public dynamic ViewCountVideoReport(string search_text, ref string errorMsg)
        {
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    if (search_text != null)
                    {
                        data = (search_text == "0" ? data : data.Where(x => x.course_category_id == search_text));
                    }
        ...
        public dynamic CoursePrint(int? user_id, string id, ref string error)
        {
            dynamic output = null;
            ...
            try
            {
                ...
                {
                    var get_role = db.user.Where(w => w.id == user_id).Select(s => s.role_id).FirstOrDefault();
                    var data = db.course.Where(x => (id_new == "0" ? x.course_category_id != null : x.course_category_id == id_new) 
                                                    && x.is_deleted == 0
                                                    && (get_role == 3 ? x.id != null : db.group_department.Where(w => w.user_id == user_id.ToString() && w.depart_id == x.department_id).Count() > 0)
                                               ).OrderByDescending(x => x.print_count).Select(x => new
                    {
                        x.name,
                        x.print_count,
                        x.batch,
        ...
        public dynamic CourseView(int? user_id, string id, ref string error)
        {
            dynamic output = null;
            ...
            try
            {
               string id_new = id == null ? "" : id;
                ...
                using (DataContext db = new DataContext())
                {
                    ...
                    var filter_course = db.course.Where(x => (id_new == "0" ? x.course_category_id != null : x.course_category_id == id_new) 
                                                            && x.is_deleted == 0
                                                            && (get_role == 3 ? x.id != null : db.group_department.Where(w => w.user_id == user_id.ToString() && w.depart_id == x.department_id).Count() > 0)
                                                        );
        ...
        public dynamic get_report_topten(int id, string category_id, DateTime? start_date, DateTime? end_date, ref string error)
        {
            dynamic output = null;
            var data = new List<modelTopTen>();
            var mockData = new List<modelTopTen>();
            ...
            using (var db = new DataContext())
            {
                try
                {
                    if(id == 1)
                    {
                        mockData = (from c in db.course
                                    where c.is_deleted == 0 && (category_id == "0" ? c.course_category_id != null : c.course_category_id == category_id)
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = "(รุ่นที่ " + c.batch.ToString() + ") " + c.name.ToString(),
                                        value = db.course_lesson.Where(w => w.is_deleted == 0 && w.course_id == c.id).Sum(s => s.count_view)
                                    }).OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    }
                    else if(id == 2)
                    {
                        mockData = db.video_on_demand.Where(w => w.is_deleted == 0
                            && (category_id == "0" ? w.course_category_id != null : w.course_category_id == category_id)
                        )
                        .Select(s => new modelTopTen {
                            rank = 0,
                            title = s.name,
                            value = s.count_view
                        }).OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    ...
                    {
                        mockData = db.course.Where(w => w.is_deleted == 0 && (category_id == "0" ? w.course_category_id != null : w.course_category_id == category_id))
                            .Select(s => new modelTopTen {
                                rank = 0,
                                title = "(รุ่นที่ " + s.batch.ToString() + ") " + s.name.ToString(),
                                value = s.print_count
                            }).OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    }
                    else if (id == 4)
                    {
                        foreach (var c in db.course.Where(w => w.is_deleted == 0 && (category_id == "0" ? w.course_category_id != null : w.course_category_id == category_id)).ToList())
                        {
                            var count = (start_date != null ? db.course_evaluation_result.Where(w => w.course_id == c.id && w.course_evaluation_choices_id != 0 && w.is_deleted == 0 && w.created_dt >= start_date && w.created_dt <= end_date).Count() : db.course_evaluation_result.Where(w => w.course_id == c.id && w.course_evaluation_choices_id != 0 && w.is_deleted == 0).Count());

                            var value = (from ss in db.course_evaluation_result
                                         where ss.is_deleted == 0 && ss.course_evaluation_choices_id != 0 && ss.course_id == c.id && (start_date != null ? ss.created_dt >= start_date && ss.created_dt <= end_date : ss.created_dt != null)
                                         select new
                                         {
                                             a = db.course_evaluation_choices.Where(w => w.course_evaluation_id == ss.course_evaluation_id && w.id == ss.course_evaluation_choices_id).Select(s => s.score).FirstOrDefault()
                                         }).Select(s => s.a).Sum(s => s);

                            var result = (value == null || value == 0 ? 0 : (double)(value / count));
                            mockData.Add(new modelTopTen { rank = 0, title = "(รุ่นที่ " + c.batch.ToString() + ") " + c.name.ToString(), value = (int?)Math.Round(result) });
                        }
                        mockData = mockData.OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    }
                    else if (id == 5)
                    {
                        foreach(var c in db.course.Where(w => w.is_deleted == 0 && (category_id == "0" ? w.course_category_id != null : w.course_category_id == category_id)).ToList())
                        {
                            var get_exam = db.course_exam.Where(w => w.is_deleted == 0 && w.course_id == c.id);
                            var get_percent = db.course.Where(w => w.id == c.id).Select(s => s.passed_percent).FirstOrDefault();
                            var passed_percent = ((double)get_percent / 100);
                            var exam_pass = Math.Floor(get_exam.Count() * passed_percent);

                            var count = (from s_info in db.student_course_info
                                        join s in db.student.Where(w => w.is_deleted == 0) on s_info.student_id equals s.id
                                        where s_info.course_id == c.id
                                        select new
                                        {
                                            pass_at = db.course_exam_logging.Where(w => w.is_deleted == 0 &&
                                                                                w.is_pretest == 0 &&
                                            ...
                                            score = db.course_exam_logging.Where(w => w.student_id == s.id &&
                                                                                      ...
                                                                                      (start_date != null && end_date != null ? w.created_dt >= start_date && w.created_dt <= w.created_dt : w.id != 0)
                                                                                      && (get_exam.Where(wx => wx.id == w.course_exam_id && wx.answer == w.course_exam_answer_id).Count() > 0)).Count()
                                        }).Where(w => w.score >= exam_pass && exam_pass > 0 && (start_date != null ? w.pass_at >= start_date && w.pass_at <= end_date : w.pass_at != null)).Count();

                            mockData.Add(new modelTopTen { rank = 0, title = "(รุ่นที่ " + c.batch.ToString() + ") " + c.name.ToString(), value = count });
                        }
                    ...
                    else if (id == 6)
                    {
                        mockData = (from c in db.course
                                    ...
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = "(รุ่นที่ " + c.batch.ToString() + ") " + c.name.ToString(),
                                        value = db.student_course_info.Where(w => w.is_deleted == 0 && w.course_id == c.id && (start_date != null ? w.created_dt >= start_date && w.created_dt <= end_date : w.created_dt != null)).Count()
                                    }).OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    }
                    else if (id == 7)
                    {
                        mockData = (from p in db.provinces
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = p.province_name,
                                        value = (from s in db.student
                                                 join s_info in db.student_course_info.Where(w => w.is_deleted == 0) on s.id equals s_info.student_id
                                                 where s.province_id == p.id && s.is_deleted == 0
                                                 select new
                                                 {
                                                     s.id
                    ...
                    {
                        mockData = (from b in db.business_type
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = b.name,
                    ...
                    {
                        mockData = (from e in db.educational
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = e.name,
                    ...
                    {
                        mockData = (from k in db.know_channel
                                    select new modelTopTen
                                    {
                                        rank = 0,
                                        title = k.name,
                                        value = db.mapping_student_know_channel.Where(w => w.know_channel_id == k.id).Count(),
                                    }).OrderByDescending(o => o.value).Skip(0).Take(10).ToList();
                    }
                    ...
                    {
                        data.Add(new modelTopTen { rank = i, title = item.title, value = item.value });
                        i++;
                    }
        ...
        public dynamic student_exam_raw(string course_id, ref string error_message)
        {
            dynamic data = null;
            using (var db = new DataContext())
            {
                try
                {
                    var get_exam = db.course_exam.Where(w => w.course_id == course_id);
                    var get_exam_answer = db.course_exam_answer.Where(w => w.course_id == course_id);

                    data = (from cel in db.course_exam_logging
                            where cel.course_id == course_id
                            select new
                            {
                                user_id = cel.student_id,
                                name = db.student.Where(w => w.id == cel.student_id).Select(s => s.firstname + " " + s.lastname).FirstOrDefault(),
                                exam_type = cel.is_pretest == 1 ? "Pre-test" : "Post-test",
                                question_id = cel.course_exam_id,
                                question_text = get_exam.Where(w => w.id == cel.course_exam_id).Select(s => s.question).FirstOrDefault(),
                                answer_id = cel.course_exam_answer_id,
                                answer_text = get_exam_answer.Where(w => w.course_exam_id == cel.course_exam_id && w.order == cel.course_exam_answer_id).Select(s => s.answer).FirstOrDefault()
                            }).OrderBy(o => o.user_id).ToList();
                }
                catch (Exception ex)
                {
                    error_message = ex.Message; ErrorList(ex);
