...
text-decoration:none;font-family: Noto Sans Thai;font-style: normal;font-weight: bold;font-size: 12px;line-height: 18px;text-align: center;color: #FFFFFF;"" href=" + url + @" target=""_blank"">เข้าเรียนคอร์สนี้</a>";
   ...
   public dynamic getVideo()
        ...
        public dynamic SignatureRead(ref string errorMsg)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    var data = db.certificate.Where(x => x.is_active == "Y").ToList().Select(x => new
                    {
                        x.id,
                        x.manager_name,
                        path = fileUrl + x.path,
        ...
        public void SignatureCertificate(certificate m, Authentication.Authentication auth, ref string errorMsg)
        {
            using (var db = new DataContext())
            {
                try
                {
                    var data = db.certificate.Where(x => x.id == m.id).FirstOrDefault();
                    if (data != null)
                    {
                        data.manager_name = m.manager_name;
                        data.path = m.path;

                        data.is_active = m.is_active;
                        data.positions = m.positions;
                        db.SaveChanges();

                    }
                    else
                    {
                        var cert = db.certificate.Where(x => x.is_active == "Y").ToList();
        ...
        public dynamic DocumentTypeReadList(ref string errorMsg)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    var data = db.document_type.Where(a=>a.is_delete!=0).ToList();
                    output = new
                    {
        ...
        public void DocumentTypeCreate(document_type m,Authentication.Authentication auth, ref string errorMsg)
        {
            using (var db = new DataContext())
            {
                try {
                    var data = db.document_type.Where(a => a.id == m.id).FirstOrDefault();
                    if (data != null)
                    {
                        data.document_name = m.document_name;
                        data.is_active = m.is_active;
                        data.is_delete = m.is_delete;

                        data.update_by = auth.user_id.ToString();
                    ...
                    else
                    {
                        m.is_active = "Y";
                        m.is_delete = 0;
                        m.create_by = auth.user_id.ToString();
                        m.create_dt = DateTime.Now;
                        db.document_type.Add(m);
                        db.SaveChanges();
                    }
                } catch(Exception ex)
                {
       ...
       public void DocumentTypeUpdate(document_type m,Authentication.Authentication auth,ref string errorMsg)
        {
            try
            {
                using (var db = new DataContext()) { 
