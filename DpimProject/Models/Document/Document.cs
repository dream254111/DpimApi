                ...
                var j = 0;
                    while ((ws.Cells[start_row, 1].Value?.ToString().Trim() ?? "") != "")
                    {
                        ...
                        var d= new List<Data.DataModels.course_exam_answer>();
                            exam_h.order = Int32.Parse(ws.Cells[start_row, 1].Value?.ToString().Trim());
                        exam_h.is_deleted = 0;
                        exam_h.score= 1;
                ...
                using (var db = new DataContext())
                {
                    ...
                    using (var excel = new ExcelPackage(fi))
                    {
                        ...
                        while ((ws.Cells[start_row, 1].Value?.ToString().Trim() ?? "") != "")
                        {
                            ...
                            v.start_dt = string.IsNullOrEmpty(start_dt) || start_dt.ToString() == "" ? (DateTime?)null : DateTime.Parse(start_dt);
                            v.end_dt = string.IsNullOrEmpty(end_dt) || end_dt.ToString() == "" ? (DateTime?)null : DateTime.Parse(end_dt);
