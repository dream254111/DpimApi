...
namespace DpimProject.Models.Authentication
{
    public class Authentication
    {
        ...
        public dynamic group_department { get; set; } = null;
        ...
        public bool LogIn(string username, string password, bool remember, ref string token_text, ref string error)
        {
            ...
            try
            {
                ...
                dynamic permiss = null;
                if (m.role_id == 3)
                {
                    permiss = db.admin_menu.Where(a => !a.menu_id.Contains("10")).Select(a => new
                    {
                        a.menu_id,
                        a.menu_name,
                        a.menu_type,
                ...
                {
                    permiss = (from a in db.admin_menu
                               join b1 in db.menu_permission on new { a.menu_id } equals new { b1.menu_id } into b2
                               from b in b2.DefaultIfEmpty()
                               where !a.menu_id.Contains("10") && b.user_id == m.id.ToString()
                               select new
                               {
                                   a.menu_id,
                                   a.menu_type,
                               ...
                               }).ToList().Select(a => new
                               {
                                   ...
                                   a.menu_type,
                                   a.status,
                                   disabled = (a.status == "Y") ? false : true
                               }).ToList();
                }
                //var permiss = db.admin_menu.Where(a=>!a.menu_id.Contains("10")).ToList().Select(a => new
                //{
                //    a.menu_id,
                //    a.menu_name,
                //    a.menu_type,
                //    status = db.menu_permission.Where(b =>!b.menu_id.Contains("10")&& b.menu_id == a.menu_id && b.user_id == m.id.ToString()).Select(b => b.status).FirstOrDefault(),
                //    disabled = (db.menu_permission.Where(b => !b.menu_id.Contains("10") && b.menu_id == a.menu_id && b.user_id == m.id.ToString()).FirstOrDefault() != null) ? false : true
                //}).ToList();
        ...
        public bool checkHaveStudent(string email)
        {
            DataContext db = new DataContext();
            var student = db.user.Where(w => w.email == email && w.is_deleted == 0).Count();
                ...
                if (data != null)
                {
                    ...
                    else
                    {
                        permiss = (from a in db.admin_menu
                                   join b1 in db.menu_permission on new { a.menu_id } equals new { b1.menu_id } into b2
                                   from b in b2.DefaultIfEmpty()
                                   where !a.menu_id.Contains("10") && b.user_id == data.id.ToString()
                                   select new
                                   {
                                       a.menu_id,
                                       a.menu_type,
        ...
        public dynamic UserDpimRegister(Models.Data.DataModels.student s,ref string errorMsg)
        {
            ...
            using (var db = new DataContext())
            {
                try {
                    ...
                    var users = db.user.Where(x => x.email == s.email).FirstOrDefault();
                    ...
                    if (data != null)
                    {
                        ...
                        data.password = password;
                        data.career_id = s.career_id;
                        data.career_name = s.position;
                        data.update_at = DateTime.Now;
                    ...
                    {
                        var data2 = db.user.Where(a => a.email == s.email).FirstOrDefault();
                        ...
                        mapping_student_know_channel.know_channel_id = 4;
                        mapping_student_know_channel.student_id = (int)user.id;
        ...
        public dynamic CheckSuperAdmin(int? user_id)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    var checkSuper = db.user.Where(w => w.id == user_id).Select(s => s.role_id).FirstOrDefault();
                    
                    output = (checkSuper == 3 ? true : false);
                }
                catch (Exception ex)
                {}
            }
