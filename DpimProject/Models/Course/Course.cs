...
namespace DpimProject.Models.Course
{
    public class Course
    {
        ...
        public class modelCourseList
        {
            ...
            public bool is_always_learning { get; set; }
            public DateTime? start_learning { get; set; }
            public DateTime? start_register { get; set; }
            public bool is_has_cost { get; set; }
            public decimal? cost { get; set; }
            public DateTime? created_at { get; set; }
            public bool hasCertificate { get; set; }
            public List<modelListinstructor> list_instructor { get; set; }

            public class modelListinstructor
            {
        ...
        public dynamic get_all_course(int? user_id, ref string error_message)
        {
            dynamic output = null;
            try
            {
        ...
        public dynamic new_batch_course(string course_id, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                course.contact_phone2 = get_course.contact_phone2;
        ...
        public dynamic get_overview_course(string id, ref string error_message)
        {
            ...
            try
            {
                ...
                output = new model_course
                {
                    ...
                    contact_phone2 = get_data.contact_phone2,
                    cost = get_data.cost,
        ...
        public dynamic save_add_course(model_course data, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                course.contact_phone2 = data.contact_phone2;
        ...
        public dynamic save_edit_course(model_course data, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                if (get_data != null)
                {
                    ...
                    get_data.contact_phone2 = data.contact_phone2;
                        ...
                        list_instructor = (from cl in db.course_lesson
                                           ...
                                           .Select(group =>
                                            new {
                                                Name = group.Key,
                                                count = group.Count(),
                                                instructor = group.ToList()
                                            })
                                            .OrderByDescending(o => o.count)
                                            .Select(sss => sss.instructor.FirstOrDefault())
                                           .ToList()
                    }).OrderByDescending(o => o.created_at).ToList();
                    
                    output = data.OrderByDescending(o => o.created_at);
                    if (sort != "" && sort != null)
                    {
                        switch (sort)
                        {
        ...
        public dynamic get_my_course_progress(Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                if (get_student != null)
                {
                    var get_info = db.student_course_info.Where(w => w.student_id == get_student.id && w.is_deleted == 0).OrderByDescending(o => o.update_dt).Skip(0).Take(2).ToList();
                    foreach (var item in get_info)
                    {
                        var get_course_lesson = db.course_lesson.Where(w => w.is_deleted == 0 && w.course_id == item.course_id).OrderBy(o => o.order);
                        var count_course = get_course_lesson.Count();
                        ...
                        var check_con = continue_learning.Where(w => w.is_done == false).OrderBy(o => o.order).FirstOrDefault();
                        if (check_con == null || (continue_learning.Count < count_course))
                        {
                            var order_lesson = check_con == null ? 1 : continue_learning.OrderByDescending(o => o.order).Select(s => s.order).FirstOrDefault();//continue_learning.order;
                            var progress_lesson = check_con == null ? (((double)continue_learning.Count() / count_course) * 100) : continue_learning.OrderByDescending(o => o.order).Select(s => s.progress).FirstOrDefault();//continue_learning.progress;
                            progress_lesson = Math.Round(progress_lesson);
        ...
        public dynamic get_course_by_id(string course_id, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                if ((check_my_own != null && check_my_own.learning_startdate == null && check_my_own.learning_enddate == null) || (check_my_own != null && check_my_own.learning_startdate <= date_now && check_my_own.learning_enddate >= date_now))
                {
                    ...
                    can_use_evaluation = db.course_evaluation.Where(w => w.is_deleted == 0 && w.course_id == course_id).Count() == 0 ? false : can_use_evaluation;
                    #endregion
                    ...
                    post_test_pass = (percent_post_test >= course.passed_percent ? true : false);
                    exam_post_test = percent_post_test >= course.passed_percent ? new List<object>() : exam_post_test;

                    can_use_evaluation = post_test_pass ? can_use_evaluation : false;
                    #endregion
        ...
        public dynamic post_send_answer_exam(bool is_pretest, string course_id, List<model_exam_logging> answer, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        is_pass = true;
                        var check_eva = db.course_evaluation.Where(w => w.course_id == course_id && w.is_deleted == 0).Count();
                        var check_cert_course = db.course.Where(w => w.id == course_id && w.hasCertificate == 1).Count();
                        if(check_eva == 0 && check_cert_course > 0)
                        {
                            ...
                            if (check_cert == null)
                            {
                                ...
                                var errorMsg = "";
                                student.GeneratedCertificateFinal(get_student_id.id, course_id, ref errorMsg);
                                manage.SendingEmail(get_student_id.id, course_id, "04", null, auth, null, ref errorMsg);
                            }
                        }
                    }
                }
        ...
        public dynamic post_send_answer_evaluation(string course_id, List<model_evaluation_result> answer, Authentication.Authentication auth, ref string error_message)
        {
            ...
            try
            {
                ...
                var check_cert = db.certificate_info.Where(w => w.course_id == course_id && w.student_id == get_student_id.id && w.cert_status == "Y").FirstOrDefault();
                var check_cert_course = db.course.Where(w => w.id == course_id && w.hasCertificate == 1).Count();
                if (check_cert == null && check_cert_course > 0)
                {
                    var student = new Student();
                    var manage = new Management.Management();
            ...
            try
            {
                ...
                var data = (from s_info in db.student_course_info
                            ...
                            }).ToList().Select(x=>new
                            {
                                ...
                                x.count_lesson,
                                x.course_time,
                                learning_end_date = $"{x.learning_end_date?.Date} {DataTools.Dtl.thai_month_sort(x.learning_end_date?.Month)} {x.learning_end_date?.Year+543}",
        ...
        public dynamic check_file_course(string filename, int type_column, int type_file, ref string error_message)
        {
            ...
            try
            {
                ...
                {
                    case 1:
                        get_data = type_file == 1 ? (db.course.Where(w => w.is_deleted == 0 && (w.cover_pic.Contains(filename) || w.info_cover.Contains(filename))).Count()) : (db.course.Where(w => w.is_deleted == 0 && w.video_sample.Contains(filename)).Count());
                        output = get_data > 1 ? false : true;
                        break;
                    case 2:
                        get_data = db.course_lesson.Where(w => w.is_deleted == 0 && w.main_p_original.Contains(filename)).Count();
                        output = get_data > 1 ? false : true;
                        break;
                    case 3:
                        get_data = type_file == 1 ? (db.course_lesson_exercise.Where(w => w.is_deleted == 0 && w.image.Contains(filename)).Count()) : (db.course_lesson_exercise.Where(w => w.is_deleted == 0 && w.video.Contains(filename)).Count());
                        output = get_data > 1 ? false : true;
                        break;
                    case 4:
                        get_data = type_file == 1 ? (db.course_exam.Where(w => w.is_deleted == 0 && w.image.Contains(filename)).Count()) : (db.course_exam.Where(w => w.is_deleted == 0 && w.video.Contains(filename)).Count());
                        output = get_data > 1 ? false : true;
                        break;
                    default: break;
                }
