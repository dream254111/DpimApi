...
namespace DpimProject.Models
{
    public class Student
    {
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    if(status_email && status_id_card)
                    {
                        ...
                        if (s.pdpa_accept == true)
                        {
                            student.pdpa_accept = "Y";
                            student.pdpa_stamp = DateTime.Now;
                        }
                        else
                        {
                            student.pdpa_accept = "N";
                        }
        ...
        public dynamic CheckEmail(string email, ref string errorMsg)
        {
            dynamic output = null;
            using (var db = new DataContext())
            {
                try
                {
                    var check_email = db.user.Where(w => w.username == email || w.email == email).FirstOrDefault();
                    output = check_email == null ? true : false;
                }
                catch (Exception ex)
                {
                    errorMsg = ex.Message;
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    //{
                        ...
                        get_student.title_name = s.title_name;
                        get_student.title_name_en = s.title_name_en;
            ...
            using (var db = new DataContext())
            {
                try
                {
                    ...
                    var profile_setting = db.manage_profile.Select(s => new
                    {
                        ...
                        is_edit_upload_front_id_document = s.is_edit_upload_front_id_document == 1 ? true : false,
                        is_edit_upload_front_back_document = s.is_edit_upload_front_back_document == 1 ? true : false,
                        is_edit_upload_selfie_document = s.is_edit_upload_selfie_document == 1 ? true : false,
                        is_edit_upload_business_document = s.is_edit_upload_business_document == 1 ? true : false,
                    }).FirstOrDefault();
            ...
            using (var db = new DataContext())
            {
               ...
               string course_id = "";
                ...
                if (percent > percent_pass && is_prestest == 0)
                {
                    ...
                    var manage = new Management.Management();
                    GeneratedCertificateFinal(auth.student_id, course_id, ref errorMsg);
            ...
            {try
                {
                    var data = (from a in db.certificate_info
                                ...
                                from b in b2.DefaultIfEmpty()
                                where a.student_id == student_id && b.is_deleted == 0 && a.cert_status=="Y"
                                select new
                                {
                                    a.certificate_id,
                                    a.certificate_dt,
       ...
       public dynamic CheckCoursePermission(string course_id,Authentication.Authentication auth ,ref string error)
        ...
        public void GeneratedCertificateFinal(int? student_id,string course_id,ref string errorMsg)
        {
            var virtual_dir = System.Configuration.ConfigurationManager.AppSettings["Certificate"];
            var photo = new Bitmap(virtual_dir + "template_final.png");
            string CertID = "";
            ...
            using (var db = new DataContext())
            {
                ...
                //{
                    ...
                    var course = db.course.Where(a => a.id == course_id).FirstOrDefault();
                    ...
                    using (var graphics = Graphics.FromImage(bitmap))
                    {
                        ...
                        var inc = new InstalledFontCollection();
                        FontFamily[] fontFamilies = inc.Families;
                        ...
                        ft.AddFontFile(url+"fonts\\supermarket.ttf");
                        ft.AddFontFile(url+"fonts\\THSarabunNew.ttf");
                        ft.AddFontFile(url+"fonts\\SukhumvitSet-Text.ttf");
                        ft.AddFontFile(url+"fonts\\SukhumvitSet-Medium.ttf");
                        ft.AddFontFile(url+"fonts\\SukhumvitSet-Thin.ttf");
                        FontFamily[] fonts = ft.Families;
                        //throw new Exception(JsonConvert.SerializeObject(fontFamilies));
                        ...
                        Font name_font = new Font(fonts[1].Name,
                            100,
                            FontStyle.Bold,
                           GraphicsUnit.Pixel);
                        //var notoB = ft.Families[1].Name;
                        Font course_font = new Font(ft.Families[1].Name,
                            80,
                            FontStyle.Bold,
                            GraphicsUnit.Pixel);
                        Font manager_font = new Font(ft.Families[1].Name,
                            78,
                            FontStyle.Bold,
                            GraphicsUnit.Pixel);
                        Font date_font = new Font(ft.Families[2].Name,
                            78,
                            FontStyle.Bold,
                            GraphicsUnit.Pixel);
                        Font month_font = new Font(ft.Families[1].Name,
                            78,
                            FontStyle.Regular,
                            GraphicsUnit.Pixel);
                        Font day_font = new Font(ft.Families[1].Name,
                            79,
                            FontStyle.Regular,
                            GraphicsUnit.Pixel);
                        Font year_font = new Font(ft.Families[2].Name,
                            79,
                            FontStyle.Bold,
                            GraphicsUnit.Pixel);
                        Font cert_font = new Font(ft.Families[2].Name,
                            60,
                            FontStyle.Regular,
                            GraphicsUnit.Pixel);
                        //var thBold = ft.Families[3].Name; 
                        Font signatureFont = new Font(ft.Families[0].Name,
                        12,
                        FontStyle.Regular,
                        GraphicsUnit.Pixel);

                        //var thBold = ft.Families[3].Name;
                        string dir_ = System.Configuration.ConfigurationManager.AppSettings["UploadPath"];
                        Size signatureSize = new Size(85, 55);
                        var manger = db.certificate.Where(x => x.is_active == "Y").FirstOrDefault();
                        Bitmap abit;
                 
                    var dir = "";

                        if (manger.path != null) { 
                            var path_ = manger.path.Substring(0, 8);
                            dir = dir_ + path_ + "\\" + manger.path;
                            abit = new Bitmap(dir);
                        abit.MakeTransparent();
                            var dbit = new Bitmap(abit, abit.Width / 2, abit.Height / 2);

                        }
                        using (var font=new Font(fonts[1].Name,
                            18,
                            FontStyle.Regular,
                            GraphicsUnit.Pixel))
                        {
                        ...
                        var cert_color = Color.FromArgb(100, 101, 101);
                            var cert_brush = new SolidBrush(cert_color);
                            var name_color = Color.FromArgb(27, 159, 136);
                            SolidBrush b = new SolidBrush(name_color);
                            var default_color = Color.FromArgb(61, 71, 89);
                            ...
                            SizeF nameSize = graphics.MeasureString(name, name_font);
                            var course_ = course?.name;
                            var year =Dtl.arabic_to_thai((CertDate.Value.Year+543).ToString()).ToString();
                            //throw new Exception(year.ToString());
                            /*Dtl.arabic_to_thai((CertDate.Value.Year + 543).ToString().Trim());*/
                            SizeF size = graphics.MeasureString(course_.ToString(), course_font);
                            ...
                            var cert = $"{Dtl.arabic_to_thai(CertID)}/{year}";
                        var certType = $"เลขที่ {cert}";
                        var dpim = $"dpimacademy.dpim.go.th";
                        SizeF certSize = graphics.MeasureString(certType, cert_font);
                        SizeF dpimSize = graphics.MeasureString(dpim, cert_font);
                            float course_h = size.Height;
                        float course_w = size.Width;
                            SizeF daySize = graphics.MeasureString(CertDate.Value.Date.ToString() ?? "", date_font);
                            SizeF monthSize = graphics.MeasureString(Dtl.thai_month(CertDate.Value.Month), month_font);
                            SizeF yearSize = graphics.MeasureString(year, year_font);
                            if (bitmap.Width < size.Width)
                        {
                             course_w = (size.Width+150)/2;

                            //throw new Exception("Width:" + size.Width.ToString());

                            course_h = size.Height * 3;
                            }
                            var date = "ออกให้ ณ วันที่";
                            var dateSize = graphics.MeasureString(date.ToString().Trim(), course_font);
                            //throw new Exception("Size Width:" + size.Width.ToString()+" " + "bitmap Width:" + bitmap.Width.ToString());
                            RectangleF nameRect = new RectangleF((bitmap.Width-nameSize.Width)/2, 1050, nameSize.Width, nameSize.Height);
                            RectangleF drawRect = new RectangleF((bitmap.Width-course_w)/2, 1500,course_w, course_h);
                            RectangleF dateRect = new RectangleF(1210, drawRect.Y + 270, dateSize.Width, dateSize.Height);
                        //throw new Exception("Rect.X:" + dateRect.Right + " " + "Width:" + dateSize.Width.ToString());
                            RectangleF dayRect = new RectangleF(1655, drawRect.Y+291, daySize.Width, daySize.Height);
                        //Pen blackPen = new Pen(Color.Black);

                        //graphics.DrawRectangle(blackPen, dateRect.X, dateRect.Y,dateRect.Width, dateRect.Height);

                        RectangleF monthRect = new RectangleF(1780, drawRect.Y + 270, monthSize.Width, monthSize.Height);
                            RectangleF yearRect = new RectangleF(2060, drawRect.Y + 291, yearSize.Width, yearSize.Height);
                            //RectangleF dateRect = new RectangleF((bitmap.Width - dateSize.Width) / 2, drawRect.Y + 35, dateSize.Width, dateSize.Height);

                            RectangleF certRect = new RectangleF( 3000,2430 , certSize.Width, certSize.Height);
                            RectangleF dpimRect = new RectangleF( 50,2430 , dpimSize.Width, dpimSize.Height);
                            StringFormat drawFormat = new StringFormat();
                      
                            drawFormat.Alignment = StringAlignment.Center;
                       
                            ////graphics.DrawRectangle(blackPen, 5, 196, bitmap.Width - 10, course_h);
                            graphics.DrawString(date, day_font, d, dateRect, stringFormat);
                            graphics.DrawString(name, name_font, b, (photo.Width - nameSize.Width) / 2, 1050);
                            graphics.DrawString(course_, course_font, d, drawRect, drawFormat);
                            graphics.DrawString(Dtl.arabic_to_thai(CertDate.Value.Day.ToString() ?? ""), date_font,d, dayRect);
                            graphics.DrawString(Dtl.thai_month(CertDate.Value.Month),month_font,d,monthRect,drawFormat);
                            graphics.DrawString(year, year_font,d,yearRect,drawFormat);
                            //graphics.DrawString(date, course_font, d, dateRect, drawFormat);
                            //graphics.DrawString(cert, num_cert, d, certRect, drawFormat);

                            var manager_name = $"({manger.manager_name})";
                            var managerSize = graphics.MeasureString(manager_name, manager_font);
                            RectangleF managerRect = new RectangleF((bitmap.Width-managerSize.Width)/2, 2200, managerSize.Width, managerSize.Height);
                            SizeF positionSize = graphics.MeasureString(manger.positions, course_font);

                            RectangleF positionRect = new RectangleF((bitmap.Width-positionSize.Width)/2, 2300, positionSize.Width, positionSize.Height);
                            graphics.DrawString(manager_name, manager_font, d, managerRect, drawFormat);
                            graphics.DrawString(manger.positions, manager_font, d, positionRect, drawFormat);
                            graphics.DrawString(certType, cert_font, cert_brush, certRect, drawFormat);
                            graphics.DrawString(dpim, cert_font, cert_brush, dpimRect, drawFormat);
                        if (!string.IsNullOrEmpty(dir))
                        {
                            using (Image objGraphic = System.Drawing.Image.FromFile(dir))
                            {

                                RectangleF signatureRect = new RectangleF(1450, dayRect.Y + 150, 664, 272);
                            
                                graphics.DrawImage(objGraphic, signatureRect);
                    ...
                    new_path = virtual_dir + course_id + "\\" + student_id + "_" + CertID + "_" + CertDate.Value.Year.ToString() + ".png";
                    var d1 = db.certificate_info.Where(x => x.student_id == student_id && x.course_id == course_id && x.cert_status == "Y").FirstOrDefault();

                    if (d1 != null)
                    {
                        d1.path = student_id+"_"+CertID + "_" + CertDate.Value.Year.ToString() + ".png";
                        db.SaveChanges();
                    }
                    else
                    {
                        var s = new certificate_info
                        {
                            ...
                            created_dt = CertDate,
                            path = student_id + "_" + CertID + "_" + CertDate.Value.Year.ToString() + ".png",
                    cert_status = "Y"
                        };
        ...
        public void ResetGenereteCertificate(int? student_id, string course_id,string certificate_id, ref string errorMsg)
        {
            var virtual_dir = System.Configuration.ConfigurationManager.AppSettings["Certificate"];
            var photo = new Bitmap(virtual_dir + "template_final.png");
            string CertID = "";
            ...
            using (var db = new DataContext())
            {
                ...
                var data = (from a in db.certificate_info
                            ...
                            join c1 in db.course on new { a.course_id } equals new { course_id = c1.id } into c2
                            from c in c2.DefaultIfEmpty()
                            where a.student_id == student_id && a.course_id == course_id&&a.certificate_id==certificate_id
                            select new
                            {
                                //a.certificate_id,
                                //a.certificate_dt,
                    ...
                    DateTime? CertDate = data?.certificate_dt;
                    var path = virtual_dir + course.id.ToString() + "\\";
                //errorMsg = path;
                //throw new Exception(path);
          
                FileInfo file = new FileInfo(path+data.path);
                    //throw new Exception(path.ToString() + data.path.ToString());
                if (file.Exists)
                {
                    file.Delete();
                }
                ...
                var d1 = db.certificate_info.Where(x => x.student_id == student_id && x.course_id == course_id && x.cert_status == "Y").FirstOrDefault();
                    ...
                    if (d1 == null)
                    {
                        ...
                        d1.update_by = student_id;
                        d1.update_dt = DateTime.Now;
                        db.SaveChanges();
                    }
        ...
        {
            var virtual_dir = System.Configuration.ConfigurationManager.AppSettings["Certificate"];
            var photo = new Bitmap(virtual_dir + "template1.png");
            string CertID = "";
            DateTime? CertDate = DateTime.Now;
            Bitmap bitmap = new Bitmap(photo.Width,photo.Height);
            var new_path = "";
            using (var db = new DataContext())
            {
                ...
                using (var graphics = Graphics.FromImage(bitmap))
                {
                    ...
                    ft.AddFontFile(url + "fonts\\Sarabun\\Sarabun-Bold.ttf");
                        ...
                        FontFamily[] fonts = ft.Families;
                        //throw new Exception(JsonConvert.SerializeObject(ft));
                    graphics.Clear(Color.White);
                    graphics.DrawImage(photo, new Rectangle(0, 0, photo.Width, photo.Height), 0, 0, photo.Width, photo.Height, GraphicsUnit.Pixel, attributes);
                    Font noto = new Font(ft.Families[2].Name,
                        12,
                        FontStyle.Regular,
                        GraphicsUnit.Pixel);
                    //var notoB = ft.Families[1].Name;
                    Font thSarabun = new Font(ft.Families[1].Name,
                        ...
                        FontStyle.Regular,
                        GraphicsUnit.Pixel);
                    Font sarabun = new Font(ft.Families[1].Name,
                        14,
                        FontStyle.Regular,
                        GraphicsUnit.Pixel);
                        //var thBold = ft.Families[3].Name; 
                        Font signatureFont = new Font(ft.Families[1].Name,
                        12,
                        FontStyle.Regular,
                        GraphicsUnit.Pixel);
                        //var thBold = ft.Families[3].Name;
                        ...
                        if (manger == null)
                        {
                            dir = virtual_dir + "signature_default.png";
                        }
                        else
                        {
                            var path_ = manger.path.Substring(0, 8);
                        ...
                        using (var font = new Font("Sarabun", 10, FontStyle.Regular))
                    {
                        ...
                        if (bitmap.Width < size.Width)
                        {
                            ...
                            RectangleF nameRect = new RectangleF(2, 170, nameSize.Width, nameSize.Height);
                        RectangleF drawRect = new RectangleF(5,196, bitmap.Width-20, course_h);
                        RectangleF dateRect = new RectangleF((bitmap.Width - dateSize.Width) / 2, drawRect.Y + 35, dateSize.Width, dateSize.Height);
    ...
    RectangleF certRect = new RectangleF((bitmap.Width - certSize.Width) / 2, dateRect.Y + 17, certSize.Width, certSize.Height);
                        ...
                        graphics.DrawString(cert, sarabun, d, certRect, drawFormat);
                            using (Image objGraphic = System.Drawing.Image.FromFile(dir))
                            {
                                RectangleF signatureRect = new RectangleF((bitmap.Width - 67) / 2, certRect.Y + 15, 67, 47);
                                if (manger != null)
                                {
                                    var manager_name = $"( {manger.manager_name} )";
                                    var n = graphics.MeasureString(manger.positions, sarabun);
                                    SizeF managerSize = graphics.MeasureString(manager_name, sarabun);

                                    RectangleF managerRect = new RectangleF((bitmap.Width - managerSize.Width) / 2, signatureRect.Y + 50, managerSize.Width, managerSize.Height);
                                    RectangleF managePostion = new RectangleF((bitmap.Width - n.Width) / 2, managerRect.Y + 20, n.Width, n.Height);
                                    //throw new Exception("ManagerRect;"+JsonConvert.SerializeObject(managerRect)+ "SignatureRect" + JsonConvert.SerializeObject(signatureRect));
                                    graphics.DrawString(manager_name, sarabun, d, managerRect, drawFormat);
                                    graphics.DrawString(manger.positions, sarabun, d, managePostion, drawFormat);
                                }
                                else
                                {
                                    signatureRect = new RectangleF((bitmap.Width - 218) / 2, certRect.Y+20, 224, 122);
                                }
                                graphics.DrawImage(objGraphic, signatureRect);

                            }

                            //graphics.DrawImage(dbit, signatureRect);
...
enddate=((x.learning_enddate??DateTime.Now.AddDays(30))-DateTime.Now).TotalDays<30?true : false
        ...
        public dynamic get_all_banner(ref string errorMsg)
        {
            ...
            try
            {
                using (var db = new DataContext())
                {
                    var data = db.banner.Where(w => w.is_deleted == 0 && w.active == 1).OrderBy(o => o.order).ToList();
                    output = data;
                }
            }
            catch (Exception ex)
        ...
        public dynamic get_special_days(ref string errorMsg)
        {
            ...
            using (var db = new DataContext())
            {
                try
                {
                    var data = db.special_days.Where(w => w.is_deleted == 0 && (w.start_date <= DateTime.Now && w.end_date >= DateTime.Now || w.start_date == null)).FirstOrDefault();
                    output = data;
                }
                catch (Exception ex)
                {
