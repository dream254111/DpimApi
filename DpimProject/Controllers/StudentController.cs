...
namespace DpimProject.Controllers
{
    ...
    public class StudentController : ApiController
    {
        ...
        [HttpPost]
        public HttpResponseMessage CheckEmail()
        {
            string error = "";
            var m = new
            {
            ...
            var data = student.CheckEmail(m.email, ref error);
        ...
        public async Task<HttpResponseMessage> Login()
        {
            ...
            if (f.username.Contains("@dpim.go.th"))
            {
                ...
                error = await DpimAuthentication(username, f.password, f.username);
            }
        ...
        [HttpPost]
        public async Task<string> DpimAuthentication(string username, string password, string email)
        {
            string error = "";
            var f = new
            {
            ...
            try
            {
                ...
                if (d1["user"] == null)
                {
                    var checkStudent = auth.checkHaveStudent(email);
                    error = checkStudent ? "รหัสผ่านไม่ถูกต้อง" : "ไม่พบข้อมูลในระบบกรุณาเช็ค email or password";
                }
                else
                {
        ...
        public HttpResponseMessage GenereteCertificate(string course_id)
        {
            ...
            if (string.IsNullOrEmpty(error))
            {
                student.GeneratedCertificateFinal(auth.user_id, course_id, ref error);
        ...
        [ActionName("ResetGenereteCertificate")]
        [HttpGet]
        public HttpResponseMessage ResetGenereteCertificate(string course_id,string certificate_id)
        {
            string error = "";
            ...
            if (string.IsNullOrEmpty(error))
            {
                student.ResetGenereteCertificate(auth.user_id, course_id,certificate_id, ref error);
